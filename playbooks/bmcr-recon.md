...
**Three ways a row gets wrongly recorded as a verified zero.** Each shipped before being caught: (1) **a broken reader's zero** — an OCR path that fails on a missing credential still returns 0, and the row reads as "we agree there was nothing on the invoice"; put the error marker in the unread list. (2) **A row excluded from the scorer entirely** — rows the catalogue cannot score are rightly skipped, but skipping them left them with no verdict, no label and no place on the review tab, so they vanished from the analysis instead of being reported as unevaluable. *"We cannot check this" is a finding; silence is not* — and such a row can carry a real award. (3) **The wrong invoice scored out of a bundled PDF** — see the entry below, which corrects an earlier reading of this case.

**Exclude by COVERAGE, not by a proxy attribute.** A scorer skipped every wholesale-account invoice on the documented grounds that wholesale line items use a SKU family absent from the retail product catalogue, so those rows "always score zero". Measured across 40 such invoices: the absent family accounts for 68% of line items — and **25% of line items match the catalogue fine**. So the scorer returns a *partial* read, not a zero. That is worse than a zero and is the real reason to exclude: a partial total systematically undercounts, lands below what the manufacturer awarded, and settles as *"they credited at least what we found"* — a verified-looking settle on an invoice a quarter of which was read.

The proxy also fails in the other direction: a **single-line** wholesale invoice parses completely, and one such row scored 10 points against an award of zero — a real under-award the account-level exclusion was hiding. Account type was standing in for catalogue coverage, and it is wrong on exactly the rows that *are* checkable. The better rule is to score the row and refuse only when some line item falls outside the catalogue — coverage is the actual criterion, so test it directly rather than through a correlate.

**Watch for an inherited claim hardening through repetition.** That "cannot be scored" line came from a project hand-off document, was never tested, and was restated perhaps half a dozen times across one working session — into a process doc, a skill, and a *pushed public commit* — before anyone asked whether it had been verified. Each restatement made it feel more established; none of them was evidence. Two habits help: when you first repeat an inherited claim, say where it came from ("the hand-off doc says...") rather than asserting it flat, and treat any claim that has become load-bearing for a decision as owing a measurement, however old and settled it looks.

**The output of a capability you do not have is not a measurement.** A parser that reads photographed receipts through a vision API had no credential configured, so the call threw, the row scored zero eligible product, and the pipeline recorded *"we agree there was nothing on this invoice"* about an invoice nobody had read. The zero was real; it measured the missing credential, not the receipt. **When a tool, API, credential or integration is unavailable, its result is UNKNOWN — never zero, empty, or false.** Route it to a human, flag it, or fail loudly; never let it enter a dataset as a finding. This failure mode recurs across tools — the same organisation had a directory-audit tool return zeros because two APIs were not enabled on it.

Three practices follow. **Check for the credential up front rather than catching the exception** — a caught auth error becomes a note like *"Could not resolve authentication method"* attached to a business record, which tells the reader nothing about what to do; check first and emit an instruction instead (*"photographed receipt, no reader configured; read it by hand"*). **The moment you add an "unavailable" marker, add it to whatever list means "unverified"** — one such marker was in that list and a sibling was not, so on a machine missing the optional package those rows would have settled as verified zeros. And **deciding not to buy a capability is a legitimate choice** — the volume here was a few rows a month at well under a dollar a year — but deciding that and then letting the gap read as a finding is not.

**A "no credit awarded" status does not mean every credited FIELD is zero.** A zero-award status means zero *points* — that is what the status says. The manufacturer still credits **volume** on many of those rows (an "inverse anomaly" of several rows per statement, documented in the process notes before the code was written). Treating awarded-volume as zero by definition produced *"owed 15 gallons, none awarded"* on a row where exactly 15 had been awarded, and put five false claims in front of the reviewer. Diff every quantity against what was actually credited; never infer one field's value from another field's status.

**Never raise a zero-award row as a dispute without checking the sibling record.** Where a retailer passes billing through to the manufacturer's national account, one purchase is recorded twice and the reward rides the national-account record. A comparison of one record to one statement row cannot see that, so the retailer row reads as owed. Before flagging, look for a national-account transaction on the same work order; if one exists, downgrade to **a question, not a dispute** — "check the work order" — and never auto-close, because whether that credit covers this invoice is a two-document judgement. One work order held seven credited siblings worth 1,280 points against a 60-point claim.

**Put no conditions on that sibling.** Requiring it to be already-approved-with-points borrows a rule — *an open state is not evidence that credit is coming* — that governs **closing** a row as No Action, i.e. using the sibling to settle something. A guard that merely asks a human to look settles nothing, and a sibling still pending is **more** reason to check, not less: its credit may be about to land, which is precisely what would make the dispute wrong. Name each sibling with its status and let the reviewer judge.

**The reasoning error behind both of the above, stated generally: a rule that is true in one context applied where its logic does not hold.** "Awarded is zero" is true of points, not volume. "An open state is not evidence" is true when closing, not when questioning. Both produced defects within an hour of each other, and both put false claims in front of a reviewer. **Before reusing a rule, check that the thing which made it true still applies.**

**Build a test's sample from the source, never from the output of the step under test.** A stress test written to measure whether a parser was selecting the right document out of a bundle reported perfectly clean — 0 wrong selections. It was worthless: its sample came from the parser's own output file, so the handful of records that had failed to parse at all, the exact population at risk, could not appear in it. Re-run against the full work set, it immediately found two records parsed off the wrong document.

This is not staleness and not circularity — it is **survivorship filtering**. A step's output is by definition the set of inputs that step already handled, so measuring the step against it can only confirm what already worked. Before trusting a clean result, ask whether a failing case could even appear in the sample; if the sample was produced by the thing being tested, it usually cannot. And check the counts: 348 rows tested against 354 staged was the tell, read past because the clean answer was the expected one.

**When a check compares two sets, its two mismatch directions usually mean different things.** Comparing *records still in an unresolved state* against *records on the review tab*: one direction (in the system, missing from the tab) is a genuine defect — nobody is looking at that record. The other (on the tab, no longer in that state) means a reviewer has acted on it, which is the tab working as designed. Reporting both as failures trains people to ignore the check. Fail on the first, report the second as a note naming the record and its new state.

**Pick which invoice to parse by the AMOUNT, never by an identifier alone.** A bundled PDF holds several invoices and the parser must choose one. Choosing by the CRM's stored reference selects catastrophically badly on exactly the rows that need help: that field is wrong on every mis-transcribed row, and when the wrong value happens to be a *real* invoice elsewhere in the same bundle, the parser confidently scores somebody else's purchase. One row scored "owed 560 points / 24 gallons" off a **$0.00** page; its real invoice — total matching the transaction to the cent — contained no eligible product at all. A second scored a $130.46 invoice for a $38.16 purchase.

Switching to the statement's invoice number is not the fix, because the statement is the other party's transcription and is also wrong sometimes (one recorded `B240965` as `8240965`). **Neither identifier can arbitrate; the amount can.** Take both as candidates, keep the block whose printed total matches the transaction amount, and — critically — **refuse to score when none matches or two do**, flagging the row for a human instead. A zero produced from the wrong invoice is indistinguishable from a verified zero, so guessing is worse than declining. Measured across 344 rows: 22 bundled PDFs, 2 previously mis-scored, 0 regressions, and one row previously stuck at "could not read" resolved by amount alone. Use a tolerance of a few cents rather than equality — the two sides' totals differ by tax rounding (10c seen).

**Scoping that fix to "bundled" documents is what let it miss the live damage.** The guard above was written as a *bundle* problem, so it opened by returning early whenever the file held only one invoice — a single-invoice document was assumed to be the right one. It need not be: **the wrong document can be attached to the record outright**, and then there is no bundle, nothing is chosen, and nothing is ever checked. Both records the guard skipped were already in front of the reviewer as disputes, and both were false — each had been scored against a *different, already-credited* invoice, so the "shortfall" was volume the manufacturer had in fact paid, on another record. Both attached files printed a total of $0.00 against a real transaction amount: the contradiction was sitting in the document, unread, because the exemption meant nobody looked.

Re-measured across every cached receipt after removing the exemption: of 344, **332 scored clean, 8 carried an informational note, 4 were held** — the 2 false disputes, plus 2 that had been scoring zero off a document with no readable invoice header and were therefore *verified zeros nobody had verified*. **No record that previously scored correctly changed.** The cost of checking the other 318 was nothing, because the whole path is local text extraction. **When a guard is cheap, an exemption buys you nothing and costs you the cases you did not imagine** — and the population you exempt is invisible in every measurement of the guard's accuracy.

**The attached file was NAMED for the right invoice and contained a different one.** Both original attachments were recoverable afterwards (unlinked, not deleted), and each carried the transaction's own correct invoice number as its title while holding someone else's invoice inside. The record therefore looks correct to anyone reviewing it — the mismatch is invisible until the file is opened. **This is the argument for arbitrating on content rather than any identifier:** a title, filename, or reference check would have passed both of these. It is also not a fixed offset — one wrong invoice was adjacent to the right one, the other 27 apart — so it is the wrong page drawn from a bundle, not a predictable slip.

Note the shape of the underlying data error, which is worth checking for directly: **same vendor, same day, same submission batch** — receipts from one batch get filed onto each other's records. Resist sharpening that into "sequential numbers": it fit the first example and failed on the second, which was 27 apart. The batch is the real boundary; adjacency is a coincidence of the sample.

**Treating MISSING evidence as CONTRARY evidence produced two more defects in the same change.** When no total could be parsed off the document — or the record carried no amount at all — the amount could not arbitrate, but the code fell through to its "total contradicts the amount" branch and refused, printing the absent amount as `None` in the note: asserting a conflict with a number it did not hold. Separately, a block whose header pattern found no invoice number has an **absent** identifier, not a disagreeing one, yet the note announced that the references "did not agree" on 37 records — noise, and false. Before calling anything a contradiction, check you actually hold both sides of it. Absence is a third state and needs its own branch.

**Review the rendered packet, not just the logic that builds it.** Three defects in the reviewer-facing tab survived unit tests, adversarial replay, and a pre-flight, and were all caught within minutes by a person reading the sheet and asking what a cell meant. (1) The tab's row filter keyed on one warning marker, so every row a guard downgraded to a *question* marker silently appeared on **no review tab at all** — a guard that moves a row off the review pile is worse than the false finding it prevented; key such a filter on "not settled" so a new marker is included by default rather than excluded by omission. (2) A row whose document could not be read still rendered **0** in the "our read" columns — a number indistinguishable from a verified zero, displayed beside a staged write, which is precisely how a verification artifact gets mistaken for a write input. Blank the columns that represent *your* read; leave the counterparty-derived columns alone, since those are known whether or not you could read anything. (3) The human-readable reason for a row's presence had been computed for weeks and **never written into the packet at all** — it appeared once only because a one-off patch script added it, so the pipeline's own output silently lacked it.

On that third point: when you translate internal codes into a sentence for a reviewer, **pass anything unrecognised through verbatim**. A translator that drops codes it does not know will hide exactly the novel failure most worth seeing. And keep notes that report the parser *succeeding* out of the sentence entirely.

**A reviewer's note is INPUT, not just something to render.** The same false dispute had already been diagnosed by a person: a note on the review tab read *"the file is uploaded to the wrong record"* and named the record, while the dispute tab's own note cell for that record was blank. Two tabs, one record, never joined — so the reviewer was asked to explain a row she had already explained. The pipeline carried those notes across regenerates faithfully and **never once read one**. Add a guard that downgrades a dispute to a *question* when any human note concerns the record, and key it on **both** the note's own row and any record identifier the note's **text** names: on the measured packet, 52 of 68 notes sat on rows carrying no identifier of their own, so text-matching was the half that did the work. Measured standalone against the packet: 1 of 10 disputes withdrawn — the one that was genuinely false — and no false withdrawals across 21 noted records. **Read that figure carefully**: in the shipped system the arithmetic guard reaches that same record first and re-routes it before the note guard sees it, so the note check's real value is not the row it was built from. It is the rows where the arithmetic passes and a person still knew something — which is exactly why the two are worth keeping separate, and why a standalone measurement overstates what a second net adds on top of the first.

Keep it deliberately **orthogonal** to the arithmetic guard rather than merging them: the amount check catches records with no note, the note check catches what a person saw that the amount cannot. Two nets with different holes beat one clever net. And when a new guard fires on a broad condition because you cannot yet tell which cases matter, **ship it broad and schedule the noise review** — tightening it on the single example that revealed the bug is precisely how the arithmetic guard came to exempt the case that later cost the most.

**An earlier diagnosis of this same case blamed a pass-through twin pattern, and was wrong.** The record was not a pass-through at all: its real (non-zero) amount identified it as a direct purchase, which the locked rule already said. **Recording the wrong cause is worse than recording none** — it sends the next reader to check work orders for credited twins instead of looking at which invoice got parsed.
