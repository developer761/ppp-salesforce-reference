...
- **`BUSINESS_RULES.md`** — start here. Conventions and field gotchas: fiscal year, the canonical sales metric, quota = points, the GM / field-naming / `IsClosed` traps, lead-source classification, sales tax, the **validation-rule formula traps** (an Active rule is not an enforcing rule), and the SOQL / CLI traps (including the silent 50,000-row query cap).
