...
| cps_Inquiry_Details_Notes__c | Inquiry Details/Notes | Text Area(32768) | No | ⚠️ **Not the scope field — use `Inquiry_Notes__c`.** SF description reads "Field from old Org": a migration artifact created 2023-02-24 by the prior implementation partner (`cps_` prefix; sibling `cps_Account_Manager` on VoiceCall). **Never populated** — 0 values across the oldest 2,000 Leads (Dec 2023) and all 1,109 created in the last 14 days (checked 2026-09-24). The near-identical label makes it a picker trap next to the live field. |
